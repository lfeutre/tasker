...
(defun handle_info
  ...
  ((`#(EXIT ,pid ,reason) state)
   ...
   `#(noreply ,state))
  ((`#(stdout ,name ,_task ,results) state)
   (log-notice "Received result for '~s'" (list name))
   (list-comp ((<- r results))
     (io:format "~ts" (list r)))
   `#(noreply ,state))
  ((msg state)
...
(defun run (pid name task)
  ...
  ;;(exec:run task `(sync #(stderr ,pid) #(stdout ,pid) monitor))
  (let ((`#(ok (#(stdout ,results))) (exec:run task `(sync stdout))))
    (! pid `#(stdout ,name ,task ,results))))
